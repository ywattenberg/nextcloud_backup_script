...
def purge_backups(config: dict[str, dict[str, str]]):
    ...
    # Manage full backups:
    full_backups = get_newest_files(target_dir, r".*-full\.tar\.gz$")
    ...
    if num_full >= len(full_backups):
        ...
        oldest_kept_backup = full_backups[num_full - 1]
        ...
        for backup in differential:
            ...
            if parsed_time < oldest_date:
                logging.debug(f"""age of differentia backup {backup} older 
                    than oldest kept full backup. deleting...""")
